...
export class IndexedFasta {
  fasta!: LocalFile; // Local File
  fai!: LocalFile; // Local File
  path?: string;
  faiPath?: string;
  ...
  async getSequenceList() {
    return Object.values((await this._getIndexes()).id).map(
  ...
  async getSequenceSizes() {
    ...
    const vals: any = Object.values(idx.id);
   ...
   * is the sequence name
   */
  async getSequenceSize(seqName): Promise<number> {
    const idx = await this._getIndexes();
  ...
  async _fetchFromIndexEntry(indexEntry, min = 0, max) {
    ...
    const result = residues.toString('utf8').replace(/\s+/g, '');
